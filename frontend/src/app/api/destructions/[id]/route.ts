...
/**
 * Proxie PATCH vers le backend /api/destructions/{id} (validation une fois : réussie ou perdue).
...
export async function PATCH(
  ...
  const context = createProxyContext(`api/destructions/${id}`, getUserIdFromSession(session));
  const result = await proxyRequest(context, {
    method: "PATCH",
    path: `/api/destructions/${id}`,
    body: body || "{}",
    token,
  });
