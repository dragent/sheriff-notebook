...
| `/api/saisies/stock-adjust` | POST | `POST /api/saisies/stock-adjust` | Yes | Adjust **current aggregated stock** (FIFO) for a key (Sheriff de comté / Adjoint only): `{ key, removeQuantity }` |
